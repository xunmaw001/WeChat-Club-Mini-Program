        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('shetuanhuodong','报名') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross','是','huodongzhuangtai','已结束','已结束,待开始'.split(',')[0])">{{ contents.tableBtnFont == 1?'报名':'' }}</el-button>
                <el-button v-if="isAuth('shetuanhuodong','报名') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross','是','huodongzhuangtai','已结束','已结束,待开始'.split(',')[0])">{{ contents.tableBtnFont == 1?'报名':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('shetuanhuodong','报名') && contents.tableBtnIcon == 0" type="success" size="mini" @click="huodongbaomingCrossAddOrUpdateHandler(scope.row,'cross','是','huodongzhuangtai','已结束','已结束,待开始'.split(',')[0])">{{ contents.tableBtnFont == 1?'报名':'' }}</el-button>
  ...
  methods: {
    ...
    init () {
        this.sfshOptions = "是,否".split(',');
          this.$http({
            url: `option/huodongfenlei/leixing`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
